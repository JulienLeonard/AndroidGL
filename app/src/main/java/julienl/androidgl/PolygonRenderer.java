    ...
    public void draws(Polygon[] polygons, Color[] colors, float[] mvpMatrix) {
        ...
        for (Polygon poly : polygons) {
            ...
            for (short order: poly.GLOrder()) {
                drawOrder[norderoffset + orderindex] =  (short)(ncoordoffset/3 + order);
                orderindex += 1;
            }

            ncoordoffset += poly.GLCoordsSize();
        ...
        // Set color for drawing the triangle
        GLES20.glUniform4fv(mColorHandle, 1, colors[0].GLcoords(), 0);
